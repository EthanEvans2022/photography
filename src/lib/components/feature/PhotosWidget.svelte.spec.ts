...
describe('PhotosWidget', () => {
	...
	it('renders up to 8 photo thumbnails', async () => {
		...
		render(PhotosWidget, { photos });
		const thumbs = page.getByRole('link').filter({ hasText: '' });
		// 9 links total: "View All" + 8 thumbnails. Count image elements instead.
		const images = page.getByRole('img');
		await expect.element(images.first()).toBeVisible();
		expect((await images.all()).length).toBe(8);
	});
	...
	it('renders fewer than 8 thumbnails when fewer photos exist', async () => {
		...
		render(PhotosWidget, { photos });
		const images = page.getByRole('img');
		expect((await images.all()).length).toBe(3);
	});
